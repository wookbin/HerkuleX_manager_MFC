					   ...
					   DWORD dwBaud, BYTE byData, BYTE byStop, BYTE byParity )
{
	...
	//--> 포트명 저장..
	int m_iPortID = m_wPortID = wPortID;
	m_sPortName.Format(_T("\\\\.\\COM%d"), m_iPortID);
	...
	if (m_hComm == (HANDLE) -1)
	{
		AfxMessageBox((_T("fail Port open") + m_sPortName));
		return FALSE;
	}
...
UINT CCommThread::ThreadWatchComm(LPVOID param)//(CCommThread* pComm))
{
	...
	while (pComm ->m_bConnected)//포트가 연결되면 무한 루프에 들어감
	{
        ...
        if ((dwEvent & EV_RXCHAR) == EV_RXCHAR)
        {
			do
			{
				dwRead = pComm->ReadComm( buff, BUFF_SIZE); //들어온 데이터 읽어 오기 
